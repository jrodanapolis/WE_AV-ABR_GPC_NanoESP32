...
// variables for Gicar flow meter - PulsesPerRev = 2; ZeroTimeout = 1,800,000; Constrain = 1,500,000 and 300,000
...
// variables for pump tachometer - PulsesPerRev = 32; zeroTimeout = 100,000; constrain values = 40,000 and 5,000
...
int getPaddleState() {
  ...
  if (analogRead(A2) >= 570) {
    return 0;
  }
  if ((analogRead(A2) < 570) && (analogRead(A2) >= 475)) {
    return 1;
  }
  if (analogRead(A2) < 475) {
    return 2;
  }
}
void PrintPhase() {
...
void UpdatePressure() {
  vPres = (float)analogRead(A3);
  readPressure = ((vPres - 337.0) * 13.7895 / 819.0);
...
void setup() {
  ...
  pinMode(5, INPUT);
  pinMode(6, INPUT);
  attachInterrupt(digitalPinToInterrupt(A0), Pulse_Event_FM, RISING);
  attachInterrupt(digitalPinToInterrupt(4), Pulse_Event_PT, RISING);
  delay(1000);
  Serial.begin(115200);
...
void MakeCoffee() {
  ...
  if (digitalRead(A1) == LOW) {
    ...
    if (!timerRunning) {
      ClearPlot();
      shotStart = millis(); //added in place of RTC stuff below
    ...
    if (weightStopper == false) {
      ...
      UpdateShotTimer();
      UpdatePlot();
      currentTime = millis(); //added in place of RTC stuff below
      //RTC.getTime(currentTime);
      //currentTimeSeconds = currentTime.getHour() * 3600 + currentTime.getMinutes() * 60 + currentTime.getSeconds();
    }
    shotTimer = (currentTime - shotStart) / 1000; //removed "Seconds" from the end of currentTime and shotStart for no RTC, and added *1000
  }
}
void loop(void) {
  CheckButtonPress();
